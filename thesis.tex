...
The view frustum happens to be a convex 3D mesh, but is usually split into peices to create smaller convex meshes. 
...
That other point also gets added to the point buffer for the current slice in order to potentially contribute to the shape of the 2D rasterized polygon built from the slice.
...
To ensure all cells are traversed that intersect with the edges, a slightly modified 2D rasterizing algorithm is used.
