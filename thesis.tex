...
Each frame can take hours to render, and the result is a very photorealistic image that looks like it is real life itself.
...
\captionfonts
\caption[The photorealistic real-time graphics of Battlefield 3.]{The photorealistic real-time graphics of Battlefield 3.\cite{bf3}}
\label{fig:bf3}
\includegraphics[width=\textwidth]{Images/Borderlands_2.png}
\captionfonts
\caption[The comic book style of Borderlands 2.]{The comic book style of Borderlands 2.\cite{border2}}
...
It is relatively easy to avoid drawing portions of the scene that are not in the camera's field of view with view frustum culling by checking if the object is inside this frustum.\cite{vfc}
...
\captionfonts
\caption[A camera and its view frustum.]{A camera and its view frustum. A frustum is a pyramid with its top cut off. Objects within the frustum are visible.\cite{vfc-image}}
\label{fig:frustum}
\end{center}
\end{figure}
...
\captionfonts
\caption[A view of the Hangar in the environment made for this project.]{A view of the Hangar in the environment made for this project.}
\label{fig:hangar-view}
\includegraphics[width=\textwidth]{Images/Peices.jpg}
\captionfonts
\caption[Some of the modular pieces that make up the hangar.]{Some of the modular pieces that make up the hangar.}
...
\captionfonts
\caption[The detailed wall and its associated low detail occluder geometry shown in red.]{The detailed wall and its associated low detail occluder geometry shown in red.}
...
This reduces false positives in visibility tests because objects in back are not mistakenly determined to be visible if the objects in front are processed first.
...
\captionfonts
\caption[Example of a Z Buffer.]{Example of a Z Buffer.}
\label{fig:z-buffer}
\end{center}
\end{figure}
...
\captionfonts
\caption[An example of portal culling at work.]{An example of portal culling at work.  Zones A, B, C, and E are rendered.}
...
\captionfonts
\caption[The low resolution software z-buffer from a scene in Battlefield 3.]{The low resolution software z-buffer from a scene in Battlefield 3.\cite{culling-bf}}
\label{fig:softare-z}
\end{center}
\end{figure}
...
\captionfonts
\caption[The coverage buffer used in CryENGINE 3.]{The coverage buffer used in CryENGINE 3.  The red portions are the holes after reprojecting from previous frames to the current camera angle.\cite{cryengine3}}
...
\captionfonts
\caption[The overall scene partitioned by a uniform grid.]{The overall scene partitioned by a uniform grid.  The green cone represents the camera's field of view.}
...
\captionfonts
\caption[A moment in time of traversing the camera's view.]{A moment in time of traversing the camera's view.  The objects are white to show that they are only rendered to the depth buffer at this stage.  The cells of the grid within the camera's view are being traversed.  Green represents cells visible to the camera.  Red represents occluded cells.}
...
\captionfonts
\caption[The scene fully traversed.]{The scene fully traversed.}
...
\captionfonts
\caption[The scene is now rendered in full color.]{The scene is now rendered in full color.}
...
\captionfonts
\caption[A view frustum, and the objects inside occluding each other.]{A view frustum, and the objects inside occluding each other.}
...
\captionfonts
\caption[Showing an example of ray based frustum traversal.]{Showing an example of ray based frustum traversal. The cells in front get traversed multiple times.}
...
\captionfonts
\caption[The chessboard metric in action.]{The chessboard metric in action.}
...
\captionfonts
\caption[Occlusion order traversal.]{Occlusion order traversal.}
...
Even if the camera is staring straight at a wall, cells 21, 22, 23, and the others will falsely pass the occlusion queries because the wall in cell 32 has not been rendered to the z-buffer yet.

\begin{figure}
\begin{center}
\includegraphics[width=0.75\textwidth]{Images/wide-frustum.png}
\captionfonts
\caption[Incorrect traversal of angled wide view frustum.]{Incorrect traversal of angled wide view frustum.}
...
\captionfonts
\caption[Correct traversal of split angled wide view frustum.]{Correct traversal of split angled wide view frustum.}
...
\captionfonts
\caption[A view frustum within the bounds of a scene.]{A view frustum within the bounds of a scene.}
...
\captionfonts
\caption[A 2D slice of the same frustum seen from the side.]{A 2D slice of the same frustum seen from the side.}
...
\captionfonts
\caption[A row from the slice being rasterized.]{A row from the slice being rasterized.}
...
\captionfonts
\caption[Here the walls, ceiling, and floors are marked to always render during the depth pre-pass.]{Here the walls, ceiling, and floors are marked to always render during the depth pre-pass.  The fighter jets inside the hangars are marked to sometimes render.  The lamps on the floor and clamps holding the jets in place are marked to never render.}
...
\captionfonts
\caption[Here the building, underground tunnel walls, and the terrain are set to always render during the depth pre-pass.]{Here the building, underground tunnel walls, and the terrain are set to always render during the depth pre-pass.  The teapots and jets are set to sometimes render.  The red lamps on the building are set to never render.}
...
Backface culling should be disabled when rendering a cell when the camera is inside of a cell or else the cell will not be visible.
...
\captionfonts
\caption[Moving sideways can make far away objects seen through the door sometimes appear to pop in visibly if someone is really paying attention.]{Moving sideways can make far away objects seen through the door sometimes appear to pop in visibly if someone is really paying attention.
...
\captionfonts
\caption[Parts of the terrain go missing.]{Parts of the terrain go missing. Cells containing those objects never have a chance to be queried.}
...
\captionfonts
\caption[For each cell, the query visibility status and frame are stored.]{For each cell, the query visibility status and frame are stored.  Notice how some cells are marked as visible but their frame is less than the scene's frame, meaning the cell is still invisible.  It is time to requery that cell.}
...
It is able to light the scene with point lights, spot lights, and a directional volume light for lighting up large areas.
...
\captionfonts
\caption[Overview of Teapot Graveyard.]{Overview of Teapot Graveyard.}
...
\captionfonts
\caption[A view of the outdoor portion of the Teapot Graveyard map.]{A view of the outdoor portion of the Teapot Graveyard map.}
...
\captionfonts
\caption[Entrance into the Hangar area.]{Entrance into the Hangar area.}
...
\includegraphics[width=\textwidth]{Images/Hangar.jpg}
\captionfonts
\caption[A view of the underground Hangar of the Teapot Graveyard.]{A view of the underground Hangar of the Teapot Graveyard.}
\label{fig:hangar}
\end{center}
\end{figure}
...
\includegraphics[width=\textwidth]{Images/HallCorner.jpg}
\captionfonts
\caption[Side hallways of the Hangar in Teapot Graveyard.]{Side hallways of the Hangar in Teapot Graveyard.}
...
\captionfonts
\caption[Data for a scene with a query cap of 3000.]{Data for a scene with a query cap of 3000.}
...
\captionfonts
\caption[Data for a scene with a query cap of 4000.]{Data for a scene with a query cap of 4000.}
...
\captionfonts
\caption[Showing the framerate relative to the average framerate.]{Showing the framerate relative to the average framerate.  The top graph is for a query limit of 3000 and the bottom graph is for a query limit of 4000.  The horizontal lines mark the average framerates.}
...
For smaller light volumes, it is best to avoid using the hardware occlusion queries and to just render objects that are within the view frustum from the light's point of view.
...
It may be worth finding a way to turn culling off in some areas for certain points of view, especially if this is an area where the environment will not change too much.
...
\captionfonts
\caption[A grid hierarchy.]{A grid hierarchy.}
...
\includegraphics[width=0.5\textwidth]{Images/RasterizingAlgorithm/Final/Frust.png}
\captionfonts
\caption[A view frustum converted to a point edge list.]{A view frustum converted to a point edge list.}
...
\includegraphics[width=0.5\textwidth]{Images/RasterizingAlgorithm/Final/FrustSlice.png}
\captionfonts
\caption[A mesh edge list slice with the dimensions of the grid.]{A mesh edge list slice with the dimensions of the grid.}
...
\captionfonts
\caption[The slice and its points projected to 2D.]{The slice and its points projected to 2D.  The yellow lines show the convex polygon that will be rasterized after performing the monotone chain convex hull algorithm on the projected points.}
...
\includegraphics[width=0.5\textwidth]{Images/RasterizingAlgorithm/Final/FrustSliceNRowOrthoLabeled.png}
\captionfonts
\caption[A row of the slice being rasterized.]{A row of the slice being rasterized.}
...
\captionfonts
\caption[The two sides of the polygon are being clipped against the row bottom and top.]{The two sides of the polygon are being clipped against the row bottom and top.  Here the left red side is facing inward, while the right blue side is facing outward.  The active edges are boldened.}
...
\captionfonts
\caption[Now the point between two active edges is used to determine the row range when the edge orientation changes.]{Now the point between two active edges is used to determine the row range when the edge orientation changes.}
