...
This means the computer only has 33.33 milliseconds if trying to render at 30 frames per second.
...
The result may not be completely realistic and doesn't simulate the physics of light as accurately as non-real time graphics, but the resulting image still looks good.
It's possible to have non-photo realistic graphics as well like the character from Borderlands 2 in figure \ref{fig:borderlands2}, but those too require a large amount of processing power.
...
It's relatively easy to avoid drawing portions of the scene that aren't in the camera's field of view with view frustum culling by checking if the object is inside this frustum.
...
The algorithm may generate some false positives and determine that a few objects are visible when they really aren't.
...
Sometimes an algorithm can't figure out the best way to perform visibility culling in every situation so artists need to place hint objects.
...
It requires hardware that supports occlusion queries which allow the GPU to report how many pixels of an object aren't occluded by other objects.\cite{GpuGem-Queries}
